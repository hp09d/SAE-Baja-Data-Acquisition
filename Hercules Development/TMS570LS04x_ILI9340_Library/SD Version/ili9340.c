...
void ili9340FillRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color)
{
	...
	if(numPixels < ILI9340_MULTI_SIZE)
	{
		for(i=0; i<numPixels; ++i){
			if(i != 0){
				while(!(mibspiIsTransferComplete(mibspiREG1,ILI9340_16BIT_SINGLE)));
			}
			mibspiSetData(mibspiREG1,ILI9340_16BIT_SINGLE,ili9340PixBuf);
			mibspiTransfer(mibspiREG1,ILI9340_16BIT_SINGLE);
			--numPixels;
		}
	}else{
		...
		for( i = 0; i < ILI9340_XMIT_DELAY; ++i ){
	...
	while(numPixels > 0)
	{
		if(numPixels >= ILI9340_MULTI_SIZE)
		{
			...
			for(i=0;i<numPixels;++i)
			{
				...
				mibspiTransfer(mibspiREG1,ILI9340_16BIT_SINGLE);
				--numPixels;
				for( j = 0; j< ILI9340_XMIT_DELAY; ++j ){
					//Not sure why this delay is needed, but it fixes a bug...
				}
			}
		}
...
void ili9340DrawImage(uint16_t width, uint16_t height, const uint8_t * pixels){
	...
	while(numPixels > 0)
	{
		if(numPixels >= ILI9340_MULTI_SIZE)
		{
			...
			for(i=0;i<numPixels;++i)
			{
				ili9340PixBuf[0] = pixels[index++] << 8 + pixels[index++];
				if(i != 0){
					while(!(mibspiIsTransferComplete(mibspiREG1,ILI9340_16BIT_SINGLE)));
				}
				mibspiSetData(mibspiREG1,ILI9340_16BIT_SINGLE,ili9340PixBuf);
...
void ili9340DrawFile(FIL * file, uint16_t width, uint16_t height){
	//_disable_IRQ();
	if(width > ILI9340_TFTWIDTH || height > ILI9340_TFTHEIGHT)
	{
		return;
	...
	uint8_t dataBuf[2*ILI9340_MULTI_SIZE];
	f_read(file, &dataBuf[0], 2*ILI9340_MULTI_SIZE, &br);

	ili9340SetAddrWindow(x0, y0, x0+width-1, y0+height);

	for(i=0;i<ILI9340_MULTI_SIZE;++i)
	{
		ili9340PixBuf[i] = dataBuf[index++];
		ili9340PixBuf[i] |= dataBuf[index++]<<8;
	}
	numPixels -= ILI9340_MULTI_SIZE;
	...
	while(numPixels > 0)
	{
		...
		if(numPixels >= ILI9340_MULTI_SIZE)
		{
			...
			f_read(file, &dataBuf[0], 2*numPixels, &br);
			for(i=0;i<numPixels; i++)
			{
				ili9340PixBuf[0] = dataBuf[index++] << 8 + dataBuf[index++];
				if(i != 0){
					while(!(mibspiIsTransferComplete(mibspiREG1,ILI9340_16BIT_SINGLE)));
				}
				mibspiSetData(mibspiREG1,ILI9340_16BIT_SINGLE,ili9340PixBuf);
				mibspiTransfer(mibspiREG1,ILI9340_16BIT_SINGLE);
				/*ili9340WriteData(dataBuf[2*i]);		//Send last pixels one byte at a time
				ili9340WriteData(dataBuf[2*i+1]);*/
				--numPixels;
			}
			//numPixels = 0;
		}
	...
	for(row = 0; row < 8; ++row)
	{
		...
		for(col = 0; col < 9;){
			...
			if(pix & mask){
				...
				ili9340FillRect(x0, y0, blob*size, size, fill);
			}
		}
	}
}
...
void ili9340Write(uint16_t x, uint16_t y, uint8_t size, char * s, uint16_t color){
	uint16_t maxChars = (ILI9340_TFTWIDTH-x)/(size*9)-1;
	uint16_t chars = 0;
	uint16_t x_new = x;

	while( *s != '\0' ){
		if( *s != '\n' ){
			ili9340DrawChar(x_new,y,size,*s,color);
			x_new += 9*size;
			++chars;
		}
		++s;
...
void ili9340WriteFill(uint16_t x, uint16_t y, uint8_t size, char * s, uint16_t color, uint16_t fill){
	...
	while( *s != '\0' ){
		if( *s != '\n' ){
			ili9340DrawCharFill(x_new,y,size,*s,color,fill);
			x_new += 9*size;
			++chars;
		}
		++s;
